...
use leveldb_sys::{
    ...
    leveldb_iter_seek, leveldb_iter_seek_to_first, leveldb_iter_seek_to_last, leveldb_iter_valid,
...
use crate::options::{c_readoptions, ReadOptions};
...
pub struct KeyIterator<'a, 'iter, K: Key<'iter>> {
    inner: Iterator<'a, 'iter, K>,
}

/// An iterator over the leveldb keyspace.
///
/// Returns just the value.
pub struct ValueIterator<'a, 'iter, K: Key<'iter>> {
    inner: Iterator<'a, 'iter, K>,
}

/// A trait to allow access to the three main iteration styles of leveldb.
pub trait Iterable<'a, 'iter, K: Key<'iter>> {
    /// Return an Iterator iterating over (Key,Value) pairs
    fn iter(&'a self, options: ReadOptions<'a, 'iter, K>) -> Iterator<K>;
    /// Returns an Iterator iterating over Keys only.
    fn keys_iter(&'a self, options: ReadOptions<'a, 'iter, K>) -> KeyIterator<K>;
    /// Returns an Iterator iterating over Values only.
    fn value_iter(&'a self, options: ReadOptions<'a, 'iter, K>) -> ValueIterator<K>;
}

impl<'a, 'iter, K: Key<'iter>> Iterable<'a, 'iter, K> for Database<'iter, K> {
    fn iter(&'a self, options: ReadOptions<'a: 'opts, 'iter, K>) -> Iterator<'opts, 'iter, K> {
...
#[allow(missing_docs)]
pub trait LevelDBIterator<'a, 'iter, K: Key<'iter>> {
    #[inline]
    ...
    fn key(&self) -> K {
        unsafe {
            ...
            let slice = from_raw_parts(value, length as usize);
...
impl<'a, 'iter: 'a, K: Key<'iter>> Iterator<'a, 'iter, K> {
    fn new(database: &'a Database<'iter, K>, options: ReadOptions<'a, 'iter, K>) -> Iterator<'a, 'iter, K> {
        unsafe {
...
impl<'a, 'iter: 'a, K: Key<'iter>> LevelDBIterator<'a, 'iter, K> for Iterator<'a, 'iter, K> {
    #[inline]
    fn raw_iterator(&self) -> *mut leveldb_iterator_t {
        self.iter.ptr
    }
...
impl<'a, 'iter: 'a, K: Key<'iter>> KeyIterator<'a, 'iter, K> {
    fn new(database: &'a Database<'iter, K>, options: ReadOptions<'a, 'iter, K>) -> KeyIterator<'a, 'iter, K> {
        KeyIterator {
...
impl<'a, 'iter: 'a, K: Key<'iter>> LevelDBIterator<'a, 'iter, K> for KeyIterator<'a, 'iter, K> {
    #[inline]
    fn raw_iterator(&self) -> *mut leveldb_iterator_t {
        self.inner.iter.ptr
    }
...
impl<'a, 'iter: 'a,  K: Key<'iter>> ValueIterator<'a, 'iter, K> {
    fn new(database: &'a Database<'iter, K>, options: ReadOptions<'a, 'iter, K>) -> ValueIterator<'a, 'iter, K> {
        ValueIterator {
            inner: Iterator::new(database, options),
        }
    }
...
impl<'a, 'iter: 'a, K: Key<'iter>> LevelDBIterator<'a, 'iter, K> for ValueIterator<'a, 'iter, K> {
    #[inline]
    fn raw_iterator(&self) -> *mut leveldb_iterator_t {
        self.inner.iter.ptr
    }
...
impl<'a, 'iter: 'a, K: Key<'iter>> iter::Iterator for Iterator<'a, 'iter, K> {
    type Item = (K, Vec<u8>);
...
impl<'a, 'iter: 'a,  K: Key<'iter>> iter::Iterator for KeyIterator<'a, 'iter, K> {
    type Item = K;
...
impl<'a, 'iter: 'a,  K: Key<'iter>> iter::Iterator for ValueIterator<'a, 'iter, K> {
    type Item = Vec<u8>;
