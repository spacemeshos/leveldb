//! leveldb iterators
//!
//! Iteration is one of the most important parts of leveldb. This module provides
//! Iterators to iterate over key, values and pairs of both.
use leveldb_sys::{
    leveldb_create_iterator, leveldb_iter_destroy, leveldb_iter_key, leveldb_iter_next,
    leveldb_iter_seek, leveldb_iter_seek_to_first, leveldb_iter_seek_to_last, leveldb_iter_valid,
    leveldb_iter_value, leveldb_iterator_t, leveldb_readoptions_destroy,
};

use libc::{c_char, size_t};
use std::iter;

use crate::database::Database;
use crate::options::{c_readoptions, ReadOptions};

use db_key::Key;

use std::marker::PhantomData;
use std::slice::from_raw_parts;

#[allow(missing_docs)]
struct RawIterator {
    ptr: *mut leveldb_iterator_t,
}

#[allow(missing_docs)]
impl Drop for RawIterator {
    fn drop(&mut self) {
        unsafe { leveldb_iter_destroy(self.ptr) }
    }
}

/// An iterator over the leveldb keyspace.
///
/// Returns key and value as a tuple.
pub struct Iterator<'a, 'iter: 'a, K: Key<'iter>> {
    start: bool,
    // Iterator accesses the Database through a leveldb_iter_t pointer
    // but needs to hold the reference for lifetime tracking
    #[allow(dead_code)]
    database: PhantomData<&'a Database<'iter, K>>,
    iter: RawIterator,
    from: Option<&'a K>,
    to: Option<&'a K>,
}

/// An iterator over the leveldb keyspace.
///
/// Returns just the keys.
pub struct KeyIterator<'a, 'iter, K: Key<'iter>> {
    inner: Iterator<'a, 'iter, K>,
}

/// An iterator over the leveldb keyspace.
///
/// Returns just the value.
pub struct ValueIterator<'a, 'iter, K: Key<'iter>> {
    inner: Iterator<'a, 'iter, K>,
}

/// A trait to allow access to the three main iteration styles of leveldb.
pub trait Iterable<'a, 'iter, K: Key<'iter>> {
    /// Return an Iterator iterating over (Key,Value) pairs
    fn iter(&'a self, options: ReadOptions<'a, 'iter, K>) -> Iterator<K>;
    /// Returns an Iterator iterating over Keys only.
    fn keys_iter(&'a self, options: ReadOptions<'a, 'iter, K>) -> KeyIterator<K>;
    /// Returns an Iterator iterating over Values only.
    fn value_iter(&'a self, options: ReadOptions<'a, 'iter, K>) -> ValueIterator<K>;
}

impl<'a, 'iter: 'a, K: Key<'iter>> Iterable<'a, 'iter, K> for Database<'iter, K> {
    fn iter(&'a self, options: ReadOptions<'a, 'iter, K>) -> Iterator<'a, 'iter, K> {
        Iterator::new(self, options)
    }

    fn keys_iter(&'a self, options: ReadOptions<'a, 'iter, K>) -> KeyIterator<'a, 'iter, K> {
        KeyIterator::new(self, options)
    }

    fn value_iter(&'a self, options: ReadOptions<'a, 'iter, K>) -> ValueIterator<'a, 'iter, K> {
        ValueIterator::new(self, options)
    }
}

#[allow(missing_docs)]
pub trait LevelDBIterator<'a, 'iter, K: Key<'iter>> {
    #[inline]
    fn raw_iterator(&self) -> *mut leveldb_iterator_t;

    #[inline]
    fn start(&self) -> bool;

    #[inline]
    fn started(&mut self);

    fn from(self, key: &'a K) -> Self;
    fn to(self, key: &'a K) -> Self;

    fn from_key(&self) -> Option<&K>;
    fn to_key(&self) -> Option<&K>;

    fn valid(&self) -> bool {
        unsafe { leveldb_iter_valid(self.raw_iterator()) != 0 }
    }

    fn advance(&mut self) -> bool {
        unsafe {
            if !self.start() {
                leveldb_iter_next(self.raw_iterator());
            } else {
                if let Some(k) = self.from_key() {
                    self.seek(k)
                }
                self.started();
            }
        }
        self.valid()
    }

    fn key(&self) -> K {
        unsafe {
            let length: size_t = 0;
            let value = leveldb_iter_key(self.raw_iterator(), &length) as *const u8;
            let slice = from_raw_parts(value, length as usize);

            K::from(slice)
        }
    }

    fn value(&self) -> Vec<u8> {
        unsafe {
            let length: size_t = 0;
            let value = leveldb_iter_value(self.raw_iterator(), &length) as *const u8;
            from_raw_parts(value, length as usize).to_vec()
        }
    }

    fn seek_to_first(&self) {
        unsafe { leveldb_iter_seek_to_first(self.raw_iterator()) }
    }

    fn seek_to_last(&self) {
        if let Some(k) = self.to_key() {
            self.seek(k);
        } else {
            unsafe {
                leveldb_iter_seek_to_last(self.raw_iterator());
            }
        }
    }

    fn seek(&self, key: &K) {
        unsafe {
            let k = key.as_ref();

            leveldb_iter_seek(
                self.raw_iterator(),
                k.as_ptr() as *mut c_char,
                k.len() as size_t,
            );
        }
    }
}

impl<'a, 'iter: 'a, K: Key<'iter>> Iterator<'a, 'iter, K> {
    fn new(
        database: &'a Database<'iter, K>,
        options: ReadOptions<'a, 'iter, K>,
    ) -> Iterator<'a, 'iter, K> {
        unsafe {
            let c_readoptions = c_readoptions(&options);
            let ptr = leveldb_create_iterator(database.database.ptr, c_readoptions);

            leveldb_readoptions_destroy(c_readoptions);
            leveldb_iter_seek_to_first(ptr);

            Iterator {
                start: true,
                iter: RawIterator { ptr: ptr },
                database: PhantomData,
                from: None,
                to: None,
            }
        }
    }

    /// return the last element of the iterator
    pub fn last(self) -> Option<(K, Vec<u8>)> {
        self.seek_to_last();
        Some((self.key(), self.value()))
    }
}

impl<'a, 'iter: 'a, K: Key<'iter>> LevelDBIterator<'a, 'iter, K> for Iterator<'a, 'iter, K> {
    #[inline]
    fn raw_iterator(&self) -> *mut leveldb_iterator_t {
        self.iter.ptr
    }

    #[inline]
    fn start(&self) -> bool {
        self.start
    }

    #[inline]
    fn started(&mut self) {
        self.start = false
    }

    fn from(mut self, key: &'a K) -> Self {
        self.from = Some(key);
        self
    }

    fn to(mut self, key: &'a K) -> Self {
        self.to = Some(key);
        self
    }

    fn from_key(&self) -> Option<&K> {
        self.from
    }

    fn to_key(&self) -> Option<&K> {
        self.to
    }
}

impl<'a, 'iter: 'a, K: Key<'iter>> KeyIterator<'a, 'iter, K> {
    fn new(
        database: &'a Database<'iter, K>,
        options: ReadOptions<'a, 'iter, K>,
    ) -> KeyIterator<'a, 'iter, K> {
        KeyIterator {
            inner: Iterator::new(database, options),
        }
    }

    /// return the last element of the iterator
    pub fn last(self) -> Option<K> {
        self.seek_to_last();
        Some(self.key())
    }
}

impl<'a, 'iter: 'a, K: Key<'iter>> LevelDBIterator<'a, 'iter, K> for KeyIterator<'a, 'iter, K> {
    #[inline]
    fn raw_iterator(&self) -> *mut leveldb_iterator_t {
        self.inner.iter.ptr
    }

    #[inline]
    fn start(&self) -> bool {
        self.inner.start
    }

    #[inline]
    fn started(&mut self) {
        self.inner.start = false
    }

    fn from(mut self, key: &'a K) -> Self {
        self.inner.from = Some(key);
        self
    }

    fn to(mut self, key: &'a K) -> Self {
        self.inner.to = Some(key);
        self
    }

    fn from_key(&self) -> Option<&K> {
        self.inner.from
    }

    fn to_key(&self) -> Option<&K> {
        self.inner.to
    }
}

impl<'a, 'iter: 'a, K: Key<'iter>> ValueIterator<'a, 'iter, K> {
    fn new(
        database: &'a Database<'iter, K>,
        options: ReadOptions<'a, 'iter, K>,
    ) -> ValueIterator<'a, 'iter, K> {
        ValueIterator {
            inner: Iterator::new(database, options),
        }
    }

    /// return the last element of the iterator
    pub fn last(self) -> Option<Vec<u8>> {
        self.seek_to_last();
        Some(self.value())
    }
}

impl<'a, 'iter: 'a, K: Key<'iter>> LevelDBIterator<'a, 'iter, K> for ValueIterator<'a, 'iter, K> {
    #[inline]
    fn raw_iterator(&self) -> *mut leveldb_iterator_t {
        self.inner.iter.ptr
    }

    #[inline]
    fn start(&self) -> bool {
        self.inner.start
    }

    #[inline]
    fn started(&mut self) {
        self.inner.start = false
    }

    fn from(mut self, key: &'a K) -> Self {
        self.inner.from = Some(key);
        self
    }

    fn to(mut self, key: &'a K) -> Self {
        self.inner.to = Some(key);
        self
    }

    fn from_key(&self) -> Option<&K> {
        self.inner.from
    }

    fn to_key(&self) -> Option<&K> {
        self.inner.to
    }
}

impl<'a, 'iter: 'a, K: Key<'iter>> iter::Iterator for Iterator<'a, 'iter, K> {
    type Item = (K, Vec<u8>);

    fn next(&mut self) -> Option<(K, Vec<u8>)> {
        if self.advance() {
            Some((self.key(), self.value()))
        } else {
            None
        }
    }
}

impl<'a, 'iter: 'a, K: Key<'iter>> iter::Iterator for KeyIterator<'a, 'iter, K> {
    type Item = K;

    fn next(&mut self) -> Option<K> {
        if self.advance() {
            Some(self.key())
        } else {
            None
        }
    }
}

impl<'a, 'iter: 'a, K: Key<'iter>> iter::Iterator for ValueIterator<'a, 'iter, K> {
    type Item = Vec<u8>;

    fn next(&mut self) -> Option<Vec<u8>> {
        if self.advance() {
            Some(self.value())
        } else {
            None
        }
    }
}
